...
module tb_full_transformer;
    ...
    // Intent names (using SystemVerilog string for perfect formatting)
    string intent_names [0:5];
    initial begin
    ...
    initial begin
        ...
        if (pass_count == total_tests) begin
            $display(">>> ALL SYSTEMVERILOG END-TO-END ACCELERATOR INFERENCE TESTS PASSED! <<<\n");
            $finish;
        end else begin
            $fatal(1, ">>> REGRESSION FAILED: %0d MISMATCHES <<<\n", total_tests - pass_count);
        end
    end
endmodule
