...
# 16:9 Presentation Aspect Ratio (1920x1080 equivalent at 200 DPI)
...
# Header divider line (using Rectangle to prevent any Line2D clipping artifacts)
div_line = plt.Rectangle((0.05, 0.890), 0.90, 0.0015, facecolor=CYAN, alpha=0.35, edgecolor='none', transform=ax.transAxes, zorder=2)
...
# Total table width = 0.90 exactly (0.05 to 0.95)
# Column widths sum exactly to 0.90: 0.22 + 0.14 + 0.14 + 0.14 + 0.26 = 0.90
col_widths = [0.22, 0.14, 0.14, 0.14, 0.26]
col_x = [start_x]
...
table_data = [
    ...
    ["Total Latency @ 50 MHz", "113.76 µs", "111.20 µs", "111.20 µs", "Deterministic sub-ms latency"],
    ["Exponential ROM Footprint", "256 entries (16-bit)", "0 entries (NO ROM!)", "16 entries (16-bit)", "Eliminates ROM memory entirely (T0)"],
    ["Exponentiation Datapath", "256-word ROM lookup", "Pure Barrel Shifter", "16-LUT + Barrel Shift", "Zero ROM, zero DSP in Tier 0"],
    ["Extra Pipeline Stages", "2 (Descale + Requant)", "0 (Eliminated)", "0 (Eliminated)", "Eliminates 16 cycles/row (128c total)"],
    ["Mean Absolute Error (MAE)", "0.100967", "0.002312", "0.000246", "≈ 44x lower (T0) | ≈ 410x lower (T1)"],
    ["Max Absolute Error", "0.401563", "0.009236", "0.000979", "Over 400x peak error reduction (T1)"],
    ["Kullback-Leibler Divergence", "0.532291", "0.016769", "0.014876", "High probability distribution fidelity"],
    ["Rank Inversion Rate (1,000 vec)", "0.000%", "0.000%", "0.000%", "0% rank inversion (100% intent match)"],
...
# 4. Bottom Conclusion Box (Strictly within [0.05, 0.95])
bot_y = 0.145
bot_h = 0.075
...
# 5. Footer Metadata (Strictly within [0.05, 0.95])
